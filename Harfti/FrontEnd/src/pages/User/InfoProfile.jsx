...
export default function InfoProfile({ user ,Items }) {
    return (
        <div className="flex flex-col h-[auto] w-full items-center gap-y-10  ">
            ...
            <div >
                {user && <img src={`http://localhost:3001/uploads/${Items ? Items:user.image }`} className="rounded-[50%] h-30 flex justify-center " />}
            </div>
            ...
            <div className={`flex flex-col items-start gap-3  text-black  `}  >
                ...
                <div className="flex space-x-5" ><h2 className="text-black   font-bold">First Name : </h2>  <p>{user && user.firstName}</p> </div>
                <div className="flex space-x-5" ><h2 className="text-black   font-bold">Last Name:</h2><p>{user && user.lastName} </p>  </div>
